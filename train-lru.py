...
class SequenceLightningModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        ssm_list = ["nu_log", "theta_log", "gamma_log", "norm", "B"]    
        
        ssm_parameters = [p for n, p in self.named_parameters() if any(i in n for i in ssm_list)]
        regular_parameters = [p for n, p in self.named_parameters() if not any(i in n for i in ssm_list)]
    
        params = [
            {'params': ssm_parameters, 'lr': self.hparams.optimizer.lr * self.hparams.train.lr_factor, 'weight_decay': 0},
